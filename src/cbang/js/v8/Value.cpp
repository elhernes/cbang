...
bool Value::has(uint32_t index) const {
  auto ctx=getCtx();
  return value->ToObject(ctx).ToLocalChecked()->Has(ctx, index).ToChecked();
}


bool Value::has(const string &key) const {
  return value->ToObject(getCtx()).ToLocalChecked()->Has(getCtx(), createString(key)).ToChecked();
}


SmartPointer<js::Value> Value::get(int index) const {
  return new Value(value->ToObject(getCtx()).ToLocalChecked()->Get(getCtx(), index));
}


SmartPointer<js::Value> Value::get(const string &key) const {
  return new Value(value->ToObject(getCtx()).ToLocalChecked()->Get(getCtx(), createString(key)));
}


SmartPointer<js::Value> Value::getOwnPropertyNames() const {
  return new Value(value->ToObject(getCtx()).ToLocalChecked()->GetOwnPropertyNames(getCtx()));
}
...
Value Value::call(Value arg0, const vector<Value> &args) const {
  ...
  return v8::Handle<v8::Function>::Cast(value)->
    Call(getCtx(), arg0.getV8Value()->ToObject(getCtx()).ToLocalChecked(), args.size(), argv.get());
